...
plt.plot(eeg[3000:4500,[2,8]]);  #  Stimulus and P3

plt.axis('tight');
plt.title('EEG Signals of C3 channel with stimulus (Single-letter P Protocol)');
...
np.savetxt('S11LPC3.txt', classData)
...
smoothed = np.zeros_like(segments[:,:,2])
for trial in range(80):
    smoothed[trial,:] = np.linalg.solve(DI,segments[trial,:,2])
'''
stimulus = eeg[:,-1]
...
np.savetxt('S11LPC3-smoothed.txt', classData4smoothed)
