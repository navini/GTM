...
plt.plot(eeg[3000:4500,[6,8]]);  #  Stimulus and P3

plt.axis('tight');
plt.title('EEG Signals of O1 channel with stimulus (Single-letter P Protocol)');
...
np.savetxt('S11LPO1.txt', classData)
...
smoothed = np.zeros_like(segments[:,:,6])
for trial in range(80):
    smoothed[trial,:] = np.linalg.solve(DI,segments[trial,:,6])
'''
stimulus = eeg[:,-1]
...
np.savetxt('S11LPO1-smoothed.txt', classData4smoothed)
