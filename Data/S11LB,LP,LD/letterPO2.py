...
plt.plot(eeg[3000:4500,[7,8]]);  #  Stimulus and P3

plt.axis('tight');
plt.title('EEG Signals of O2 channel with stimulus (Single-letter P Protocol)');
...
np.savetxt('S11LPO2.txt', classData)
...
smoothed = np.zeros_like(segments[:,:,7])
for trial in range(80):
    smoothed[trial,:] = np.linalg.solve(DI,segments[trial,:,7])
'''
stimulus = eeg[:,-1]
...
np.savetxt('S11LPO2-smoothed.txt', classData4smoothed)
