...
plt.plot(eeg[3000:4500,[3,8]]);  #  Stimulus and P3

plt.axis('tight');
plt.title('EEG Signals of C4 channel with stimulus (Single-letter P Protocol)');
...
np.savetxt('S11LPC4.txt', classData)
...
smoothed = np.zeros_like(segments[:,:,3])
for trial in range(80):
    smoothed[trial,:] = np.linalg.solve(DI,segments[trial,:,3])
'''
stimulus = eeg[:,-1]
...
np.savetxt('S11LPC4-smoothed.txt', classData4smoothed)
