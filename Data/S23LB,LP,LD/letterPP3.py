...
data = json.load(open('s23-gammasys-gifford-unimpaired.json','r'))
summarize(data)
letterp = data[4]
...
plt.axis('tight');
plt.title('EEG Signals of 8 channels with stimulus (Single-letter P Protocol)');
plt.show()
...
plt.plot(eeg[3000:4500,[4,8]]);  #  Stimulus and P3

plt.axis('tight');
plt.title('EEG Signals of P3 channel with stimulus (Single-letter P Protocol)');
...
targetLetter = letterp['protocol'][-1]
...
np.savetxt('S23LPP3.txt', classData)
...
smoothed = np.zeros_like(segments[:,:,4])
for trial in range(80):
    smoothed[trial,:] = np.linalg.solve(DI,segments[trial,:,4])
'''
stimulus = eeg[:,-1]
...
np.savetxt('S23LPP3-smoothed.txt', classData4smoothed)
