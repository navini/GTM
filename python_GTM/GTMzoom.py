...
def drawSignal(fig, classData, plotData, xmin, ymin, xmax, ymax, color):
    print 'in draw signal ',xmin,' ',xmax,' ',ymin,' ',ymax

    for i in classData:
        if(plotData[i,0]>xmin and plotData[i,0]<xmax and plotData[i,1]>ymin and plotData[i,1]<ymax):
            axes2 = fig.add_axes([(plotData[i,0]-xmin)/(xmax-xmin)*0.8+0.1, (plotData[i,1]-ymin)/(ymax-ymin)*0.8+0.1, 0.05,0.05])
            axes2.set_alpha(0.0)
            axes2.plot(data_features[i,:],color)
            axes2.axis('off')
...
def zoomRedraw(event, fig, classesBN, c1, classesPN, c2, classesDN, c3, classesB, c4, classesP, c5, classesD, c6, means):
    print plt.connect(event,re_zoom)  # for rectangle-select zoom
    drawSignal(fig, classesBN, means, xmin, ymin, xmax, ymax, c1)
    drawSignal(fig, classesPN, means, xmin, ymin, xmax, ymax, c2)
    drawSignal(fig, classesDN, means, xmin, ymin, xmax, ymax, c3)
    drawSignal(fig, classesB, means, xmin, ymin, xmax, ymax, c4)
    drawSignal(fig, classesP, means, xmin, ymin, xmax, ymax, c5)
    drawSignal(fig, classesD, means, xmin, ymin, xmax, ymax, c6)'''
...
'''
drawSignal(fig, classesBN, means, xmin, ymin, xmax, ymax, 'r')
drawSignal(fig, classesPN, means, xmin, ymin, xmax, ymax, 'y')
drawSignal(fig, classesDN, means, xmin, ymin, xmax, ymax, 'c')

drawSignal(fig, classesB, means, xmin, ymin, xmax, ymax, 'b')
drawSignal(fig, classesP, means, xmin, ymin, xmax, ymax, 'g')
drawSignal(fig, classesD, means, xmin, ymin, xmax, ymax, 'm')'''
...
def re_zoom(event):
    ...
    changed = None
    print fig.get_axes()
    print '66666333311115555 ',axes1.get_xlim()
    xmin, xmax = axes1.get_xlim()
    ymin, ymax = axes1.get_ylim()
    print xmin,' ',xmax,' ',ymin,' ',ymax
    zoom = [[0.0,0.0],[0.0,0.0]]    # Zoom from each end of axis (2 values per axis)
