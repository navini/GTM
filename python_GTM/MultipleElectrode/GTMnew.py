...
def gtmpost(nin, covars, gmm_c, ncentres, priors, rbf_c, X, wi, w2, b2, data):
    ...
    #print 's',s
    s = np.where(s>0,s,1)
    lol = np.dot(s,np.ones((1, ncentres)))
    #print 'min value of S', s[s!=0].min(),s.min(),s.max(),s[s!=0].max(),post[post!=0].max(),post.max(),post.min(),post[post!=0].min()
    #print 'divide value', 0.0/s[s!=0].min()
    for i in range(post.shape[0]):
        for j in range(post.shape[1]):
            #print 'i',i,'  j',j,'  s',lol[i,j],'  post',post[i,j]
            #print post[i,j]/lol[i,j]
            post[i,j]=post[i,j]/lol[i,j]
...
data1=np.loadtxt('S11LBC3-smoothed.txt')
data2=np.loadtxt('S11LBC4-smoothed.txt')
data3=np.loadtxt('S11LBF3-smoothed.txt')
data4=np.loadtxt('S11LBF4-smoothed.txt')
data5=np.loadtxt('S11LBO1-smoothed.txt')
data6=np.loadtxt('S11LBO2-smoothed.txt')
data7=np.loadtxt('S11LBP3-smoothed.txt')
data8=np.loadtxt('S11LBP4-smoothed.txt')
#print '###',data.shape
data = np.hstack((data7,data8[:,1:211],data1[:,1:211],data2[:,1:211]))
#,data4[:,1:211],data5[:,1:211],data6[:,1:211],data7[:,1:211],data8[:,1:211] ,data3[:,1:211]
print data.shape
data_features = data[:,1:841]
...
print PCcoeff
print 'PCvec'
print PCvec[:, 0:gtm_dimLatent]
'''
PCvec = np.hstack((PCvec[:, 0:1]*(-1),PCvec[:, 1:2]))
print 'PCvec'
print PCvec[:, 0:gtm_dimLatent]'''
#print 'sqrt'
...
for i in classes0:
    ...
    #axes2 = fig.add_axes([(means[i,0]-(-1))/2*0.8+0.1-0.01, (means[i,1]-(-1))/2*0.8+0.1-0.01, 0.05,0.05])
    #axes2.set_alpha(0.0)
    axes1.plot(means[i,0], means[i,1], 'r.')
...
for i in classes1:
    #axes2 = fig.add_axes([(means[i,0]-(-1))/2*0.8+0.1-0.01, (means[i,1]-(-1))/2*0.8+0.1-0.01, 0.05,0.05])
    #axes2.set_alpha(0.0)
    axes1.plot(means[i,0], means[i,1],'bx')
