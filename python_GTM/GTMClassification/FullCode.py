...
gmm_centres = np.random.randn(gmm_ncentres,gmm_nin)   # Initialized from random normal distribution
...
for i in range(data_classes.shape[0]):
    if data_classes[i,0]==0:
        lo=axes1.plot(means[i,0], means[i,1], 'r.')
        lo=axes1.plot(modes[i,0], modes[i,1], 'rx')
    ...
    if data_classes[i,0]==1:
        ...
        lx=axes1.plot(modes[i,0], modes[i,1], 'bx')
...
axes1.set_title('Modes on Latent space')
fig2.show()
#plt.show()
fig3 = plt.figure(figsize=(9,9))
axes1 = fig3.add_axes([0.1, 0.1, 0.8,0.8]) # main axes
...
axes1.set_title('Means on Latent space')
fig3.show()
